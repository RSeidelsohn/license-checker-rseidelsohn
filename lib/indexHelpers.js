...
/**
 * This function has a wanted side effect, as it modifies the json object that is passed by reference.
 *
...
export function deleteNonDirectDependenciesFromAllDependencies(
...
export function getFirstNotUndefinedOrUndefined(...args) {
...
export function getAuthorDetails({ clarification, author }) {
	const publisher = getFirstNotUndefinedOrUndefined(clarification?.publisher, author?.name);
	const email = getFirstNotUndefinedOrUndefined(clarification?.email, author?.email);
	const url = getFirstNotUndefinedOrUndefined(clarification?.url, author?.url);

	return { publisher, email, url };
}

export function getLinesWithCopyright(fileContents = '') {
	return fileContents
		.replace(/\r\n/g, '\n')
		.split('\n\n')
...
export const getModuleNameForLicenseTextHeader = (moduleName = '') => {
...
export const storeReadmeInJsonIfExists = (modulePath, currentExtendedPackageJson) => {
	if (
