...
const recursivelyCollectAllDependencies = (options) => {
    ...
    if (json.author && typeof json.author === 'object') {
        ...
        let publisher = getFirstNotUndefinedOrUndefined(clarification?.publisher, json?.author?.name);
        if (mustInclude('publisher') && publisher) {
            moduleInfo.publisher = publisher;
        }

        let email = getFirstNotUndefinedOrUndefined(clarification?.email, json?.author?.email);
        if (mustInclude('email') && email) {
            moduleInfo.email = email;
        }

        let url = getFirstNotUndefinedOrUndefined(clarification?.url, json?.author?.url);
    ...
    if (options.customFormat) {
        Object.keys(options.customFormat).forEach((item) => {
            if (mustInclude(item) && moduleInfo[item] == null) {
                moduleInfo[item] = getFirstNotUndefinedOrUndefined(
                    clarification?.[item],
...
/**
 * The "direct" option can be set to true, false or a number.
 * This function transforms the direct value to a number that
 * is used as the depth value in the readInstalledPackages function.
 */
const transformDirectValueToDepthValue = (directValue) => {
    if (directValue === true) {
        return 0;
    }
...
exports.init = function init(args, callback) {
    args.direct = transformDirectValueToDepthValue(args.direct);
