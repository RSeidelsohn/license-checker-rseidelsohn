...
const init = (args, callback) => {
	...
	readInstalledPackages(args.start, optionsForReadingInstalledPackages, (err, installedPackagesJson) => {
		...
		const failOnOnlyAllowHits = []

		const checkIfCheckHits = (packageName) => {
		...
		const checkForFailOn = (currentLicense) => {
			...
			if (toCheckforFailOn.includes(currentLicense)) {
				const failMessage = `Found license defined by the --failOn flag: "${currentLicense}".`

				if (args.failSlow)  {
					failOnOnlyAllowHits.push(failMessage)
				} else {
					console.error(`${failMessage} Exiting.`);
					process.exit(1);
				}
			}
		};
		...
		const checkForOnlyAllow = (currentLicense, packageName) => {
			if (toCheckforOnlyAllow.length > 0) {
				...
				if (!containsOneOfAllowedPackages) {
					const failMessage = `Package "${packageName}" is licensed under "${currentLicense}" which is not permitted by the --onlyAllow flag.`
		...
		Object.keys(resultJson).forEach(checkIfCheckHits);

		if (failOnOnlyAllowHits.length > 0) {
			console.error(failOnOnlyAllowHits.join('\n'));
			process.exit(1);
		}
