...
import readInstalledPackages from './readInstalledPackagesWithArborist.js';
import { writeOutput } from './util/output.js';
...
const recursivelyCollectAllDependencies = options => {
	...
	if (mustInclude('url')) {
		...
		const url = helpers.getFirstNotUndefinedOrUndefined(clarification?.url, currentExtendedPackageJson?.url?.web);
		if (url) {
			moduleInfo.url = url;
		}
	}
	...
	if (licenseData) {
		...
		if (Array.isArray(licenseData) && licenseData.length > 0) {
			...
			moduleInfo.licenses = licenseData.map(moduleLicense => {
				const moduleLicenseTypeOrName = helpers.getFirstNotUndefinedOrUndefined(moduleLicense.type, moduleLicense.name);
...
/**
 * @typedef {Object} InitOpts Options struct for the init() and runLicenseCheck() functions.
 ...
 * @property {(string | ModuleInfo)=} customPath To add a custom Format file in JSON.
 ...
 * @property {CustomFormat=} customFormat Specify the columns for CSV format or add the specified items for JSON format.
...
/**
 * @typedef {Object} ModuleInfo Information about one dependency.
 ...
 * @property {(string | string[])=} licenses Array of licenses.
 ...
 *
 * @param {InitOpts} options Controls the license checker's behavior.
...
export const runLicenseCheck = async options => {
	...
	if (options.clarificationsFile) {
		const clarificationsFromFile = parseJson(options.clarificationsFile);
	...
	if (optionsForReadingInstalledPackages.depth === 0) {
		helpers.deleteNonDirectDependenciesFromAllDependencies(installedPackagesJson, options);
	}
	...
	const filterDeletePrivatePackages = privatePackage => {
		if (resultJson[privatePackage]?.private) {
	...
	const onlyIncludeWhitelist = (whitelist, filtered) => {
		...
		Object.keys(filtered).forEach(filteredPackage => {
			...
			if (
				whitelist.findIndex(whitelistPackage =>
					filteredPackage.startsWith(whitelistPackage.lastIndexOf('@') > 0 ? whitelistPackage : `${whitelistPackage}@`)
				) !== -1
			) {
		...
		Object.keys(filtered).forEach(filteredPackage => {
			...
			if (
				blacklist.findIndex(blacklistPackage =>
					filteredPackage.startsWith(blacklistPackage.lastIndexOf('@') > 0 ? blacklistPackage : `${blacklistPackage}@`)
				) === -1
			) {
				resultJson[filteredPackage] = filtered[filteredPackage];
			}
	...
	const excludePackagesStartingWith = (blacklist, currentResult) => {
		...
		for (const pkgName in resultJson) {
			for (const denyPrefix of blacklist) {
				if (pkgName.startsWith(denyPrefix)) {
					delete resultJson[pkgName];
				}
			}
		}
	...
	// biome-ignore lint/complexity/useMaxParams: Don't want to change behavior at this moment
	...
	Object.keys(allWantedDepthDependenciesWithVersions)
		...
		.forEach(item => {
			...
			if (allWantedDepthDependenciesWithVersions[item]) {
				if (options.relativeModulePath && allWantedDepthDependenciesWithVersions[item].path != null) {
		...
		if (Array.isArray(excludeLicenses) && excludeLicenses.length > 0) {
			Object.entries(sorted).forEach(([packageName, packageData]) => {
				...
				if (!licenses) {
					...
					const licenseMatch = getLicenseMatch(licensesArr, resultJson, packageName, packageData, excludeLicenses);
		...
		if (Array.isArray(includeLicenses) && includeLicenses.length > 0) {
			Object.entries(sorted).forEach(([packageName, packageData]) => {
				...
				if (!licenses) {
					...
					const licensesArr = Array.isArray(licenses) ? licenses : [licenses];
					const licenseMatch = getLicenseMatch(licensesArr, resultJson, packageName, packageData, includeLicenses);
	...
	// package whitelist
	const whitelist = helpers.getOptionArray(options.includePackages);
	if (whitelist) {
		resultJson = onlyIncludeWhitelist(whitelist, resultJson);
	}

	// package blacklist
	const blacklist = helpers.getOptionArray(options.excludePackages);
	if (blacklist) {
		resultJson = excludeBlacklist(blacklist, resultJson);
	}

	// exclude by package name starting with a string
	const excludeStartStringsArr = helpers.getOptionArray(options.excludePackagesStartingWith);
	...
	writeOutput(options, resultJson);
	return resultJson;
};
 ...
 *
 * @param {InitOpts} args Specifies the path to the module to check dependencies of.
