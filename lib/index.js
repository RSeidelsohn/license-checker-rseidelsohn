...
export const runLicenseCheck = args => {
	...
	let resolveLicenseCheck = () => {};
	let rejectLicenseCheck = () => {};
	const licenseCheckPromise = new Promise((resolve, reject) => {
		resolveLicenseCheck = resolve;
		rejectLicenseCheck = reject;
	});

	readInstalledPackages(args.start, optionsForReadingInstalledPackages, (err, installedPackagesJson) => {
		if (err) {
			debugError(err);
			rejectLicenseCheck(err);
			return;
		}
		...
		if (noPackagesFoundError) {
			...
			rejectLicenseCheck(noPackagesFoundError);
			return;
		}

		// Output to files, if necessary
		writeOutput(args, resultJson);
		resolveLicenseCheck(resultJson);
	});
