...
const recursivelyCollectAllDependencies = (options) => {
    ...
    if (mustInclude('repository')) {
        ...
        if (repositoryUrl) {
            moduleInfo.repository = repositorUrl;
        }
    }
    ...
    if (typeof json.author === 'object') {
        const { publisher, email, url } = helpers.getAuhorDetails({ clarification, json?.author })
        /*istanbul ignore else - This should always be there*/
        let publisher = helpers.getFirstNotUndefinedOrUndefined(clarification?.publisher, json?.author?.name);
        if (mustInclude('publisher') && publisher) {
            moduleInfo.publisher = publisher;
        }

        let email = helpers.getFirstNotUndefinedOrUndefined(clarification?.email, json?.author?.email);
        if (mustInclude('email') && email) {
            moduleInfo.email = email;
        }

        let url = helpers.getFirstNotUndefinedOrUndefined(clarification?.url, json?.author?.url);
    ...
    if (json.dependencies) {
        Object.keys(json.dependencies).forEach((dependencyName) => {
            const childDependency = options.currentRecursionDepth > options._args.direct ? {} : json.dependencies[dependencyName];
