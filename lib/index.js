...
export const runLicenseCheck = async args => {
	...
	try {
		installedPackagesJson = await readInstalledPackages(args.start, optionsForReadingInstalledPackages);
	} catch (error) {
		debugError(error);
		throw error;
	}
	...
	let resultJson = {};
	const excludeLicenses = args.excludeLicenses
		?.match(/([^\\\][^,]|\\,)+/g)
		.map(license => license.replace(/\\,/g, ',').replace(/^\s+|\s+$/g, ''));
	const includeLicenses = args.includeLicenses
		?.match(/([^\\\][^,]|\\,)+/g)
		.map(license => license.replace(/\\,/g, ',').replace(/^\s+|\s+$/g, ''));
